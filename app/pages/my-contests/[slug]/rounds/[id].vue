...
const { data, error, refresh } = await useFetch(`/api/my/rounds/${route.params.id}`, {
  server: false,
  headers: computed(() => ({
    ...
    <template v-if="round && contest">
      ...
      <div class="space-y-3">
        <div class="flex items-center gap-2 text-sm text-muted-foreground">
          <NuxtLink to="/my-contests" class="hover:text-foreground transition-colors">Mis Concursos</NuxtLink>
          <span>/</span>
          <NuxtLink :to="`/my-contests/${contest.slug}`" class="hover:text-foreground transition-colors">{{ contest.name }}</NuxtLink>
          <span>/</span>
          <span class="text-foreground font-medium">{{ round.name }}</span>
        </div>

        <div class="flex flex-wrap items-start justify-between gap-4">
          <div class="flex items-center gap-3">
            <NuxtLink :to="`/my-contests/${contest.slug}`" class="p-2 hover:scale-110 transition-transform">
              <ArrowLeft class="w-5 h-5 text-muted-foreground" />
            </NuxtLink>
            <div>
              <div class="flex flex-wrap items-center gap-2">
                <h1 class="text-2xl font-bold tracking-tight">{{ round.name }}</h1>
                <Badge class="capitalize px-2 py-0.5 font-bold border-2 rounded-md text-xs" :class="getStatusClasses(round.status)">
